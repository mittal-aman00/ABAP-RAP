...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  "Now you are going to implement the method get_features which is used for
  "the instance specific dynamic feature control. It expects in the result
  "table an entry for each requested key, specifying if the related feature
  "is enabled or disabled based on the state of the instance.

  "In the present case the acceptTravel and rejectTravel actions are toggled
  "based on the current status.
      " Read the travel status of the existing travels
    READ ENTITIES OF ZI966_RAP_Travel_1 IN LOCAL MODE
      ENTITY Travel
  ...
  METHOD acceptTravel.
    "Set the travel status of the travel instances for all provided
    "Travel keys using an EML modify statement

    "Why Commit is not done after Modify Statement?
    "Ans : Inside a BO implementation (e.g. the implementation of setInitialStatus
    "or any other behavior implementation) you work - as a provider - on the transactional buffer.
    "E.g. a determination or an action may update a set of fields in the buffer.
    "In the managed scenario these buffer changes are done using EML,
    "in the unmanaged scenario this is done e.g. using some BAPIs/Function Modules.
    "All this is taking place in the "Interaction Phase" and may NEVER perform a
    "Commit Work / Commit Entities in its implementation (you should get a short dump
    "if you try it).
    "As a consumer you use EML/OData to interact on an BO from the outside.
    "And the consumer decides when a commit should happen. That is the reason why
    "we had to add a COMMIT ENTITIES in the EML unit.
    "For OData scenarios the commit is automatically done by the framework for each changeset.

    "For the MODIFY ENTITIES statement we can use IN LOCAL MODE to be able
    "to manipulate read-only fields. This makes sense e.g. for the TravelStatus
    " as this one is defined as read-only, but we have the "Accept Travel" Action,
    " which should be able to change the value.

    " Set the new overall status
    MODIFY ENTITIES OF ZI966_RAP_Travel_1 IN LOCAL MODE
    ENTITY Travel
    UPDATE
    FIELDS ( TravelStatus )
    ...
    "After modifying the travel status, the action needs to provide
    "the result as defined in the behavior definition.
    "In our present case, the result is $self which means, you must return
    "the travel instance with all values for the given keys.

    "The EML READ in LOCAL MODE, performs the select on the CDS view
    ""WITH PRIVILEGED ACCESS", skipping the access control.
  ...
  "The implementation will read the travel status for all provided keys and
  "sets the status to open for those that don’t have a status yet.
  ...
  METHOD validateAgency.
    ...
    "derive an internal table with all distinct AgencyID and perform a
    "database select to validate the existence.
    ...
    "And finally loop over the travel table, check whether an AgencyID has been provided
    "and if it exists, and raise state messages if an invalid AgencyID was provided.
    "If the AgencyID is either empty or does not exist in the check-table, a message
    "is raised using our exception class. When you make use of the so-called state messages,
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-AgencyID IS INITIAL OR NOT line_exists( agencies_db[ agency_id = travel-AgencyID ] ).
        ...
        APPEND VALUE #( %tky        = travel-%tky
                        %state_area = 'VALIDATE_AGENCY'
                        %msg        = NEW zcl966_rap_bo_message_class(
                                          severity = if_abap_behv_message=>severity-error
                                          textid   = zcl966_rap_bo_message_class=>agency_unknown
  ...
  "The implementation of the corresponding method validateCustomer follows a
  "very similar approach as for the validateAgency method.
  "You will read the CustomerID, check its existence on the database table and
  "raise state messages if an invalid value was provided.
     " Read relevant travel instance data
    READ ENTITIES OF ZI966_RAP_Travel_1 IN LOCAL MODE
      ENTITY Travel
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
        ...
        APPEND VALUE #(  %tky        = travel-%tky
                         ...
                         %msg        = NEW zcl966_rap_bo_message_class(
                                           severity   = if_abap_behv_message=>severity-error
                                           textid     = zcl966_rap_bo_message_class=>customer_unknown
  ...
  "The implementation of the corresponding method validateDates follows a very similar
  "approach as for the other two methods validateAgency and validateCustomer
     " Read relevant travel instance data
    READ ENTITIES OF ZI966_RAP_Travel_1 IN LOCAL MODE
      ENTITY Travel
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcl966_rap_bo_message_class(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcl966_rap_bo_message_class=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcl966_rap_bo_message_class(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcl966_rap_bo_message_class=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
   ...
   IF is_update_requested = abap_true.
         ...
         IF has_before_image = abap_true.
           ...
           IF update_granted = abap_false.
             APPEND VALUE #( %tky        = travel-%tky
                             %msg        = NEW zcl966_rap_bo_message_class( severity = if_abap_behv_message=>severity-error
                                                             textid   = zcl966_rap_bo_message_class=>unauthorized )
                           ) TO reported-travel.
           ENDIF.
  ...
  METHOD is_create_granted.
    AUTHORITY-CHECK OBJECT 'ZAMOSTAT'
      ID 'ZAMOSTAT' DUMMY
      ID 'ACTVT' FIELD '01'.
  ...
  METHOD is_delete_granted.
        IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZAMOSTAT'
        ID 'ZAMOSTAT' FIELD overall_status
        ID 'ACTVT' FIELD '06'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZAMOSTAT'
        ID 'ZAMOSTAT' DUMMY
