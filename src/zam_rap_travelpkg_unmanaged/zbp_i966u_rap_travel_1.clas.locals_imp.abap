...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  "Getting the data as entities. The data is again mapped according to our central
  "mapping, and we are using the control structure as well. We are then able to call
  "the travel update function module /DMO/FLIGHT_TRAVEL_UPDATE
  METHOD update.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<entity>).
      ...
      legacy_entity_x-_intx = CORRESPONDING zs966_rap_travel_1( <entity> MAPPING FROM ENTITY ).
  ...
  "lock method would perform a locking in order to avoid, when an update is performed,
  "that there are two updates happening at the same time.
  METHOD lock.
  ...
  "there is coding for the read by association booking, similar to the read method
  "for the Travel entity. This would read booking data from the buffer.
  METHOD rba_Booking.
  ...
  "we assume that new bookings cannot be created separately but can only in
  "conjunction with a given travel instance. The fact that new instances of
  "the bookings can only be created for a specific travel instance is considered
  "in the behavior definition by the _Booking association:
  """"""""""""""""""""association _Booking { create; }
  "This method is executed when a booking is created via the Travel parent entity.
  METHOD cba_Booking.
...
CLASS lsc_ZI966U_RAP_TRAVEL_1 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_ZI966U_RAP_TRAVEL_1 IMPLEMENTATION.
